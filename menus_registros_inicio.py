...
def menu_admin():
    ...
    ingresar_regiones = tk.Entry(ventana)
    tk.Button(ventana,text="mostrar regiones",command=ver_centros(arbol_logistico,ingresar_regiones)).pack()
    tk.Button(ventana, text="Volver", command=menu_Principal).pack()  
...
def guardar_centros():
    ...
    with open("centros.txt","a") as archivos:
...
def menu_User():
    ...
    distancia.pack()
    precio.pack()
    tk.Button(ventana,text="calcular costo").pack()
    tk.Button(ventana, text="Volver", command=menu_Principal).pack()
